...
class MssSensorOptode(MssSensor):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._p1 = numpy.polynomial.Polynomial(self.coefficients[0:2])  # Convert data to mV
        self._p2 = numpy.polynomial.Polynomial(self.coefficients[-2:])  # 0 Point correction with B0 and B1
    def raw_to_units(self, rawdata, offset):
...
class MssDeviceConfig(pydantic.BaseModel):
    offset: int = pydantic.Field(default=0,description='16bit offset, typically 0, older devices have -32768')
    sampling_freq: float = pydantic.Field(default=1024,description='The sampling frequency [Hz] of the microstructure probe')
    sensors: dict[str, typing.Union[MssSensor,MssShearSensor]] = pydantic.Field(default={},description='A dictionary of the sensors mounted to the probe')
    sensornames_ctd: dict[typing.Union[typing.Literal['cond'],typing.Literal['temp'],typing.Literal['press']], str] = pydantic.Field(default={'cond':'','temp':'','press':''},description='A dictionary to link standard ctd names to the names of the MSS config')
    pressure_sensorname: typing.Optional[str] = pydantic.Field(default=None,description='The sensorname of the pressure sensor, if None a bust guess of  will be done')
    pspd_rel_method: typing.Literal['pressure','constant','external'] = pydantic.Field(default='pressure',description='Method for the platform speed relative to the seawater, this is needed to calculate wavenumbers from the sampled data')
    pspd_rel_constant_vel: typing.Optional[float] = pydantic.Field(default=None, description='Constant velocity [m/s] used as pspd_rel, if defined by "pspd_rel_method"')

    def from_mrd(self, filename, shear_sensitivities=[-9999,-9999], offset=0):
        """
        ...
        for ctd_sensor in self.sensornames_ctd.keys():
            ...
            if len(sensorname_mss) == 0:
                ...
                sensornames = [header['mss']['channels'][ch]['name'] for ch in header['mss']['channels']]
                sensornames_casefold = [header['mss']['channels'][ch]['name'].casefold() for ch in header['mss']['channels']]
                for k in mss_standard_ctd_sensornames[ctd_sensor]: # Loop over the stanndard names
                    if k.casefold() in sensornames_casefold:
                        ...
                        logger.debug('\tFound MSS sensor {} for {}'.format(sensorname_mss,ctd_sensor))
        ...
        for ch in header['mss']['channels']:
            ...
            logger.debug('Checking Channel:{}, sensorname:{}, caltype:{}'.format(ch,sensorname,sensor_dict['caltype'].upper()))
            if sensor_dict['caltype'].upper() == 'N':  # Polynom
                if sensorname.upper().startswith('SHE'):
                    ...
                    self.sensors[sensorname] = MssShearSensor(channel=ch, name=sensorname,
                                                             coefficients=sensor_dict['coeff'],
                                                             unit=unit,
                                                             sensitivity=sensitivity)
                else:
                    logger.debug('\tAdding standard polynomial sensor {}'.format(sensorname))
                    self.sensors[sensorname] = MssSensorPoly(channel=ch,name=sensorname,coefficients=sensor_dict['coeff'],unit=unit)
            elif sensor_dict['caltype'].upper() == 'P':  # Pressure
                logger.debug('\tAdding pressure sensor {}'.format(sensorname))
                self.sensors[sensorname] = MssSensorPressure(channel=ch, name=sensorname, coefficients=sensor_dict['coeff'],unit=unit)
            elif sensor_dict['caltype'].upper() == 'V04':  # Oxygen
                logger.debug('\tAdding oxygen optode sensor {}'.format(sensorname))
                self.sensors[sensorname] = MssSensorOptode(channel=ch, name=sensorname, coefficients=sensor_dict['coeff'],unit=unit)
            elif sensor_dict['caltype'].upper() == 'N24':  # Internal temperature of oxygensensor
                logger.debug('\tAdding oxygen optode temperature sensor {}'.format(sensorname))
                self.sensors[sensorname] = MssSensorOptodeInternalTemp(channel=ch, name=sensorname, coefficients=sensor_dict['coeff'],unit=unit)
            elif sensor_dict['caltype'].upper() == 'NFC':  # Turbidity etc.
                logger.debug('\tAdding NFC sensor {}'.format(sensorname))
                self.sensors[sensorname] = MssSensorTurb(channel=ch, name=sensorname,
                                                               coefficients=sensor_dict['coeff'],unit=unit)
        #print('Header', header['channels'])

        return self

    def from_prb(self, filename, shear_sensitivities=[None, None], offset=0):
        """
