...
def get_records(config, logger=None):
    """
...
class mrd:
    ...
    def __init__(
        ...
        if filename is None:
            ...
            self.logger.info(
                " Opening file: {:s} will full dataset {:s} ".format(
                    filename, str(not (only_metadata))
                )
            )
            ...
            self.logger.debug(
                "Time offset unix:{}".format(self.config["time_offset_unix"])
            )
            # print('Config',self.config)
    ...
    def get_time_offset(self):
        ...
        if "time_offset" in self.config.keys():
            ...
            td_off = datetime.datetime.strptime(
                self.config["time_offset"], time_offset_format
            )
        ...
        if FLAG_interpolate_NAN:
            ...
            for i in range(nshear):  # Loop over all shear sensors
                ...
                if nnanshear:
                    print("Interpolating")
                    logger.debug(
                        "Interpolating nan {:d} of {:d} SHEAR #{:d}".format(
                            nnanshear, Ntot, i
                        )
                    )
                    ...
                    if sum(igood) > (RATIO_BADDATA * Ntot):
                        ...
                        logger.debug(
                            funcname
                            + "Will stop processing eps, bad data exceeds limit"
                        )
                        FLAG_BADDATA_EXCEEDED = True
        ...
        while True:
            ...
            if len(b) == 0:  # Nothing left anymore, break
                ...
                if IN_HEADER == False:
                    ...
                    for i in range(
                        1, 16, 2
                    ):  # Reading 16 bytes and treat them as words
                        bword.append(int.from_bytes(b[i : i + 2], byteorder="little"))
    ...
    def raw_to_units(self, rawdata, config=None):
        ...
        for i in range(nchannels):
            ...
            try:
                ...
                self.logger.info(
                    funcname + ":  Will use userdefined offset of {:d}".format(offset)
                )
            except:
                pass
            ...
            if channelname.startswith("SHE"):
                logger.debug(
                    funcname
                    + " Found shear sensor {:s} adding sensitivity to coefficient {:s}".format(
                        channelname, caltype
                    )
                )
                ...
                try:
                    ...
                    logger.debug(
                        "Found user defined sensitivity {:s}, will overwrite value found in the header of the file with {:f}".format(
                            sens_key, SP_sens
                        )
                    )
                    ...
                    self.logger.debug(
                        "Could not find user defined sensitivity {:s}, will value defined in the file".format(
                            sens_key
                        )
                    )
            ...
            if caltype.upper() == "N":  # Polynom
                self.logger.debug(
                    funcname
                    + "Converting {:s} with type {:s}".format(channelname, caltype)
                )
                ...
                data_tmp = self.raw_to_units_pressure(
                    rawdatac[:, i], coeff, offset=offset
                )
                data[channelname] = data_tmp
                ...
                data_tmp = self.raw_to_units_oxyoptode(
                    rawdatac[:, i], coeff, offset=offset
                )
                # data[:,i] = data_tmp
                data[channelname] = data_tmp
                ...
                logger.debug(
                    funcname
                    + " Unknown conversion type ({:s}) for channel {:s} ({:d})".format(
                        caltype, channelname, i + 1
                    )
                )
                # input('dsdfds')
    ...
    def raw_to_units_oxyoptode(self, rawdata, coeff, offset=0):
        ...
        p2 = numpy.polynomial.Polynomial(
            coeff[-2:]
    ...
    def __str__(self):
        ...
        rstr += ", Date: " + datetime.datetime.strftime(
            self.meta["date"], "%Y-%m-%d %H:%M:%S"
        )
        return rstr
