...
def register_plugin(parameter_list: list[tuple[str, type, float | str]]) -> Callable[type[T], type[T]]:
...
def get_registered_plugin_parameter_list(name:str) -> list[tuple[str, type, float | str]]:
    try:
        _, parameter_list = _SENSOR_SPEED_PLUGIN_REGISTRY[name]
    except KeyError:
        mesg = f"{name} is not registered as a sensor speed plugin. Registered plugins:\n"
...
class SensorSpeedLookupTable(SensorSpeedABC):
    ...
    def from_timeseries(self, t: Float[np.ndarray, "time"], U: Float[np.ndarray, "time"]) -> None:
        self.data["t"] = t
        self.data["U"] = U
